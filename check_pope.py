...
POPE_PATH = {
    "random": "./pope_coco/coco_50_pope_random.json",
    "popular": "./pope_coco/coco_50_pope_popular.json",
    "adversarial": "./pope_coco/coco_50_pope_adversarial.json",
}
...
parser.add_argument("--type", type=str, default="random", help="pope type, random, popular, adversarial")
...
USER: <ImageHere> {question} Provide as much detailed information as possible, including its attributes, location, or state. ASSISTANT:
'''

PROMPT_TEMPLATE_INSTRUCTBLIP = '''
<ImageHere>
{question} Provide as much detailed information as possible, including its attributes, location, or state.
'''

PROMPT_TEMPLATE_MINIGPT4 = '''
###Human: <Img><ImageHere></Img> {question} Provide as much detailed information as possible, including attributes, location, or state. ###Assistant:
'''
...
EVERY SENTENCE SHOULD CONTAINT THE GIVEN OBJECT.
...
3. Note the dining table is equivalent to the table.
4. Note the mannequins is not people.
Output only the corrected passage, without introducing extra contents.

Here are examples:
[Passage]:
It is a small red boat, possibly a fishing boat, and it is docked at a pier.
...
[Response]: 
It is a small red boat, possibly a fishing boat.
[Passage]:
There is a sports ball in the image, and it appears to be a soccer ball.
[Supplementary Information]:
right: There is a sports ball in the image.
right: There is no chair.
[Response]: 
There is no chair in the image. And there is a sports ball in the image.
[Passage]:
...
PROMPT_TEMPLATE_REFINE_ENTITY1 = """
You are given a query, a passage and supplementary information.
[Task]
You are required to correct and output the refined passage in a fluent and natural style, following these rules:
1. Correct the sentences in the passage if they are inconsistent with the supplementary information. Remove the objects that are confirmed to not exist in the supplementary information.
...
PROMPT_TEMPLATE_REFINE_ANSWER1 = """
You are given a statement and a question.
[Task]
Your task is to answer the question based on the statement. The statement is about some objects.
1. Your response should be limited to one of the following two choices: "Yes"/"No".
2. Pay attention to whether the attributes of the entity are correct, including color, quantity, position, etc.
3. Note that the table is equivalent to the dining table here.
3. Note that the vehicle is equivalent to the car here.
...
4. Note that the table is equivalent to the dining table here.
...
Yes
[Query]:
Is there a dinning table in the image?
[Passage]:
The table in the image is made of wood, and it appears to be a picnic table.
[Response]: 
Yes
[Query]:
Are there two horses in this image?
...
def get_lvlm_output(image, prompts, history=None):
    with torch.no_grad():
        res = model.generate(
...
def get_mllm_answers(sample,image,question_key="entity_questions", answer_key="answers"):
    """
    ...
    for dialogue in sample['dialogues']:
        ...
        if question_key=="entity_questions":
            ...
            for cur_qs in gen_qs:
                qs, entity = cur_qs                   
                if qs==sample['query']:
                    if sample["first_answer"]=="no" and sample["has_yes"]==False:
                        answer='no'
                    elif sample["first_score"]<0.04 and sample["first_answer"]=="yes":
                        answer='yes'
                    elif sample["first_score"]>0.11 and sample["first_answer"]=="no":
                        answer='no'
                    elif sample["first_score"]>0.065 and sample["first_answer"]=="yes":
                        answer='yes'
                    else:# same with input_desc
                        if sample['has_yes']==True:
                            answer='yes'
                        else:
                            answer='no'                           
...
def if_exist_entity(sentence,entity):
    if len(entity.split())==1:
        ...
        if entity in sentence:
            return True
        elif entity+'s' in sentence:
            return True
        elif entity+'es' in sentence:
            return True
        else:
            return False

def remove_irrelevant(res,entity):
    qs_set = set()
    output = []
    for s in res:
        qs, ent = s
        check = if_exist_entity(ent,entity)
        if entity=="person" or entity=="sports ball":
            check=True
        if check==False:
            continue
        else:
...
def get_target(sample):
    ...
    entity_str = [item for item in entity_str if item != "" and item != 'None']                            
...
def judge(sample,clip_scorer,image):
    ...
    for dialogue in sample['dialogues']:
        ...
        for torture in dialogue["answers_torture"]:
            ...
            id=id+1
    sup_info = remove_duplicates_wrong(sup_info)    
...
def get_refinement(sample):
    ...
    if sample["first_answer"]=="no" or sample["first_score"]<0.05:
       half_tag = True
    else:
        half_tag = False
    if sample['sup_info']==[]:
        ...
        for label in predict_labels:
            if "right" in label and " no " not in label:
                nli_value += 1
        ...
        if r <= threshold and entity_answer=='yes' and len(predict_labels)>2:
            sample["output"] = 'No, for wrong/len > threshold1.'
        elif r>0.66 and entity_answer=='yes':
            sample['output'] = sample['input_desc'][0]
        elif r==1:
            sample['output'] = sample['input_desc'][0]
            s = sample['output'].lower()
            s = s.split()
            if sample["split_sent_entity"] not in s and len(predict_labels)==1:
                sample['output']="Yes. "+sample['output']
            elif "no" in s and len(predict_labels)==1:
                if "no" not in predict_labels[0]:
                    sample['output']="Yes. "
        elif len(predict_labels)<=2 and r <=0.5 and half_tag==True: 
            sample["output"] = 'No, for wrong/len > threshold2.'
        elif r<0.76 and half_tag==True and len(predict_labels)>2:
            sample["output"] = 'No, for wrong/len > threshold3.'
        elif r==0 and entity_answer=='yes':
            sample["output"] = 'No, for wrong/len > threshold4.'
        else:
            sup_info="\n".join(sample['sup_info'])+ "\n"
            prompt = PROMPT_TEMPLATE_REFINE_ENTITY.format(passage=passage, sup_info=sup_info)
...
type=args.type
question_file = "./dataset/pope_coco/coco_50_pope_%s.json" % (type)
questions = [json.loads(q) for q in open(question_file, 'r')]
answer_save_path = '{}_{}.jsonl'.format(args.model,type)
tmp = len(questions)
progress_bar = tqdm(total=tmp,desc='Processing')
torture_log = "./pope_samples/pope_"+args.model+"_samples/llava_pope_"+type+"/%d.json"  
if not os.path.exists("./pope_samples"):
    os.mkdir("./pope_samples")
if not os.path.exists("./pope_samples/pope_"+args.model+"_samples"):
    os.mkdir("./pope_samples/pope_"+args.model+"_samples")
if not os.path.exists("./pope_samples/pope_"+args.model+"_samples/llava_pope_"+type):
    os.mkdir("./pope_samples/pope_"+args.model+"_samples/llava_pope_"+type)

if args.model=="llava-1.5":
    prompts=PROMPT_TEMPLATE_LLAVA
    threshold=[0.07,0.07,0.07]
elif args.model=="instructblip":
    prompts=PROMPT_TEMPLATE_INSTRUCTBLIP
    threshold=[0.08,0.0755,0.098]
elif args.model=="minigpt4":
    prompts=PROMPT_TEMPLATE_MINIGPT4
    threshold=[0.07,0.07,0.06]
else:
...
if type=="random":
    ...
    print("Wrong pope type!!!")


for idx, question in enumerate(questions):
    id = question["question_id"] 
    img_file = question["image"]
    img_id = int(img_file.split(".jpg")[0][-6:])
    text = question["text"] 
    iid=str(img_id)
    img_save = {}
    img_save["question_id"] = id    
    ...
    text = text.replace("USER: <ImageHere> ","")
    ...
    sample = {  
        'query': text,
        'split_sent_entity':entity,
        'img_path': image_path,
    }                

    print("entity:",entity)
    template = INSTRUCTION_TEMPLATE[args.model]
    qu1 = template.replace("<question>", text)
    output=get_lvlm_output(image,qu1)
    print("answer:",output)
    score=clip_scorer.get_clip_score("There is "+entity+" in the image.", raw_image)
    img_save["clip_score"]=score
    sample["first_score"]=score
    o=output.lower()
    o=o.replace('.', '')
    o=o.replace(',', '')
    o=o.split(' ')    
    if o[0]=='no':    
        sample["first_answer"]="no" 
        if score<th: 
            img_save["answer"]=output+" Without logic"
            with open(os.path.join(base_dir, answer_save_path), "a") as f:
                json.dump(img_save, f)
                f.write('\n')
            progress_bar.update()
            continue
    else:
        sample["first_answer"]="yes" 

    qu = prompts.format(question=text)
    print("question1:",qu)
    out=get_lvlm_output(image,qu)
    o=keep_before_last_period(out)
    claim=[]
    claim.append(o.strip())
        
    claim=remove_duplicates_answer(claim)
    print("output1:") 
    for c in claim:
        print(c)

    sample['input_desc']=claim
    if len(claim[0].split())<9 and 'Yes' in claim[0] and sample["first_answer"]=="yes":
        img_save["answer"]=claim[0]
        with open(os.path.join(base_dir, answer_save_path), "a") as f:
            json.dump(img_save, f)
            f.write('\n')
        progress_bar.update()
    ...
    for answer in claim:
        ...
        if 'yes' in entity_answer:
            ...
            if entity in answer and len(entity.split())>1:
                yes_answer.append(answer)
            elif entity in entity_answer and len(entity.split())==1:
                yes_answer.append(answer)
            if entity=="person":
                person_list=['man','woman','women','men']
                for p in person_list:
                    if p in entity_answer:
                        yes_answer.append(answer)
                        break
    ...
    if yes_answer==[]:
        ...
        sample['has_yes']=True
    sample["dialogues"] = [{"entity": entity}]
    sample = get_question_torture(sample)# object and feature extraction & question
    sample = get_mllm_answers(sample, image, question_key="entity_questions", answer_key="answers")
    sample = get_mllm_answers(sample, image, question_key="generated_torture_questions", answer_key="answers_torture")
    ...
    if 'Yes' in s:
        ...
        answer='No'
    elif entity in s and 'no' not in s:
        answer="Yes"
    else:       
        sample=get_yes_or_no(sample)
        answer=sample["final"]
    print("final_answer:",answer)
    img_save["answer"]=answer
    with open(torture_log % (id), 'w') as f:
